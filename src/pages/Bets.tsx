...
export const Bets = () => {
	...
	const fetchBets = async () => {
		try {
			...
			setError('No bets data to show, please log in or use Guest Check-In in order to place/view bets!');
			console.error(err);
		} finally {
			setIsLoading(false);
		}
	...
	if (error) return <div className='mx-auto w-[80%] text-center text-lg md:text-2xl mt-28'>{error}</div>;

	const styling = 'flex gap-6 text-answer md:text-lg justify-between items-center';

	return (
		<div className='flex flex-col p-2 gap-4 items-center justify-center'>
			{bets.length === 0 && <p className='flex justify-center text-xl md:text-4xl py-40 px-2'>No recorded bets at the moment.</p>}
