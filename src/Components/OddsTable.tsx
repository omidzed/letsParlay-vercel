...
export const OddsTable = ({ filteredEvents }: Props) => {
	...
	const betSlip = (event: Event, outcome: 'moneyline' | 'moneylineTwo' | '', overUnder: 'O 2.5' | 'U 2.5' | '') => {
		if (!hasToken()) {
			openModal(
				<AlertModal
	...
	const style = {
		...
		boxStyling: 'flex text-xs h-7 m-2 mt-2 md:text-xl w-[80%] justify-center items-center rounded-md bg-[#2e2e31] hover:bg-[#063d64] md:h-[28%] text-[#54D338] cursor-pointer',
		spread: 'flex text-xs h-7 m-2 mt-2 md:text-xl w-[80%] justify-center items-center rounded-md bg-[#2e2e31]  md:h-[28%] text-[#54D338] ',
		date: 'flex text-tiny md:text-thead justify-center items-center h-1/4 mb-5 w-full date-thead-container',
		thead: 'flex whitespace-nowrap text-rounds md:text-thead justify-center items-center h-1/4 mb-4 w-full date-thead-container',
	};
	...
	const odds = filteredEvents.map((event, index) => {
		...
		return (
			<div
				key={index}
				className='flex justify-center my-4 w-full lg:h-56'>
				<div className='flex w-full h-32 py-2 rounded-lg bg-[#212123e3] md:w-[90%] md:h-56'>
					<div className='flex w-1/2 md:w-[35%] text-white text-xl'>
						<div className='flex w-full flex-col '>
							<span className={`${style.date} full-width-border px-2  border border-t-0 border-r-0 border-l-0 border-[#2d2d31e3]`}>{formattedDateTime}</span>
							<span
					...
					<div className='flex w-1/2 md:w-[65%] text-white text-xl'>
						<div className='hidden md:block flex-col w-1/3 text-white text-xs'>
							<span className={`${style.thead} full-width-border px-2  border border-t-0 border-r-0 border-l-0 border-[#2d2d31e3]`}>PT SPREAD</span>
							<span className={style.spread}></span>
							<span className={style.spread}></span>
						</div>
						<div className='flex-col  w-1/2 md:w-1/3 text-white text-xs'>
							<span className={`${style.thead} full-width-border px-2 border border-t-0 border-r-0 border-l-0 border-[#2d2d31e3]`}>MONEYLINE</span>
							<span
						...
						<div className='flex-col  w-1/2 md:w-1/3  text-white text-tiny md:text-2xl'>
							<span className={`${style.thead} full-width-border border border-t-0 border-r-0 border-l-0 border-[#2d2d31e3]`}>ROUNDS +/-</span>
							<div
							...
							<div
								...
								<span className='md:pr-4'>{underOdds > 0 ? `+${underOdds}` : underOdds}</span>
							</div>
						</div>
					</div>
				</div>
