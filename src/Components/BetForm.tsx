...
export const BetForm = ({ event, outcomeIndex, overUnderIndex, pick, dateTime, status }: BetFormProps) => {
	...
	const calculatePayout = (odds: number, amount: number) => (calculateWinnings(odds, amount) + amount).toFixed(2);

	const winningsOver = selectedOverUnder ? calculatePayout(overUnderOdds, effectiveBetAmount) : '0.00';
	const winningsUnder = selectedOverUnder ? calculatePayout(overUnderOdds, effectiveBetAmount) : '0.00';
	const winnings = selectedOutcome ? calculatePayout(betOdds, effectiveBetAmount) : '0.00';

	// Determine the payout based on the type of bet
	const payout = winningsOver !== '0.00' ? winningsOver : winningsUnder !== '0.00' ? winningsUnder : winnings;
		...
		<>
			<div className='flex-col justify-center items-center px-10 py-10 pb-6 md:mx-10 max-w-fit'>
				<div className='flex justify-center gap-4'>
					<span className='basis-1/2 text-center'>{selectedOutcome?.name ?? selectedOverUnder?.name ?? 'Unknown'}</span>
					<span className='basis-1/2 text-left font-bold'>
						{selectedOverUnder ? (overUnderOdds > 0 ? `+${overUnderOdds}` : overUnderOdds.toString()) : betOdds > 0 ? `+${betOdds}` : betOdds.toString()}
					</span>
				</div>
				<div className='flex-col'>
					<form
						...
						<div className='flex gap-5 flex-nowrap justify-center mt-6 mb-2'>
							<div className='font-bold basis-1/2 text-right whitespace-nowrap'>Amount:</div>
						...
						<div className='flex justify-center gap-5'>
							<div className='font-bold basis-1/2 text-center whitespace-nowrap'>Payout:</div>
						...
						<div className='flex flex-col items-center justify-center'>
							<button
								className={`mt-7  ${isLoading ? 'bg-blue-400' : 'bg-blue-700'} text-white px-8 py-3 w-full rounded-md cursor-pointer`}
								type='submit'
								disabled={isLoading}>
								{isLoading ? 'Betting...' : 'Submit'}
							</button>
							or
							<button className='bg-green-700 px-8 py-3 rounded-md w-full text-white'>Parlay</button>
						</div>
					</form>
				</div>
			</div>
