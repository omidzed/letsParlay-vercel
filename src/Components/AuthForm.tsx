...
export const AuthForm = ({ action, onSignIn, toggleAction }: Props) => {
	...
	const handleSubmit = async (event: FormEvent) => {
		...
		const userData = action === 'sign-up' ? { username, password, name } : { username, password };
	...
	const styling = 'block w-full border-2 border-slate-400 bg-blue-100 rounded-md mb-1 h-10 px-4';
