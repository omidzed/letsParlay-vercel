...
export const SearchBox = ({ setInputValue }: SearchBoxProps) => {
	...
	useEffect(() => {
		...
		const timeoutId = setTimeout(() => {
			const filtered = suggestions.filter(suggestion => suggestion?.toLowerCase().includes(inputValueLocal?.toLowerCase()));
		...
		<div className='flex flex-col items-center justify-center relative'>
            ...
            2xl:text-lg pl-10 md:pl-16 pr-14 py-3 rounded-2xl'
				...
				<IoIosClose
					...
					className={`absolute z-20 text-gray-500 text-4xl cursor-pointer right-5 top-3 ${inputValueLocal ? '' : 'hidden'}`}
				/>
			</div>
				...
				<div className=' flex flex-col flex-nowrap items-center justify-start'>
					<ul className='absolute z-10 pt-2 w-[55%] md:w-1/3 lg:w-1/4 top-9 md:top-10  rounded-b-xl bg-white lg:text-custom 2xl:text-lg text-sm'>
						{filteredSuggestions.map((suggestion, index) => (
							<li
								key={index}
								className={`cursor-pointer text-black my-2 pl-10 md:pl-16 ${index === activeSuggestionIndex ? 'bg-blue-100' : ''}`}
								onClick={() => onSuggestionClick(suggestion)}>
								{index === activeSuggestionIndex ? suggestion.split('').map((char, i) => <span key={i}>{char}</span>) : suggestion}
							</li>
						))}
					</ul>
				</div>
