	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				...
				if (cachedEvents && new Date().getTime() - JSON.parse(cachedEvents).timestamp < 259200 * 1000) {
				...
				const response = await fetch(`https://api.the-odds-api.com/v4/sports/mma_mixed_martial_arts/odds/?regions=us&oddsFormat=american&apiKey=${apiKey.apiKey}`);
				if (!response.ok) {
				...
				const filteredData: Event[] = events.map(event => {
					...
					const apiOutcomes = event.bookmakers?.[0]?.markets?.[0]?.outcomes || [];
					return {
						id,
						commenceTime,
						outcomes: [
